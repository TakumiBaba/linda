      ...
      this.io.sockets.on('connection', (function(_this) {
        return function(socket) {
          ...
          socket.on('__linda_take', function(data) {
            var cid, option, tuple;
            tuple = data.tuple;
            option = data.option || {};
            cid = _this.tuplespace(data.tuplespace).take(tuple, option, function(err, tuple) {
              cid = null;
          ...
          socket.on('__linda_read', function(data) {
            ...
            tuple = data.tuple;
            option = data.option || {};
            cid = _this.tuplespace(data.tuplespace).read(tuple, option, function(err, tuple) {
              cid = null;
